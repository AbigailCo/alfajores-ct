...
export default function PedidoSection() {
  ...
  return (
    <section
      id="pedido"
      className="py-12 md:py-24 bg-(--c-secundary) relative overflow-hidden"
    >
      {/* Decoración de fondo */}
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="bg-white rounded-4xl md:rounded-[3rem] shadow-xl shadow-(--c-primary)/5 p-5 md:p-12 border border-white/50 backdrop-blur-sm">
          <div className="space-y-10 md:space-y-12">
            {/* 1️⃣ Selección de caja */}
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                <span className="shrink-0 flex items-center justify-center w-7 h-7 md:w-8 md:h-8 rounded-full bg-(--c-primary) text-white text-[10px] md:text-xs font-bold">
                  1
                </span>

                <h3 className="text-lg md:text-xl font-bold text-(--c-primary) uppercase tracking-wider italic">
                  Elegí tu caja
                </h3>
              </div>

              <C.SelectorCantidad caja={caja} setCaja={cambiarCaja} />
            </div>

            <hr className="border-gray-100" />

            {/* 2️⃣ Selección de sabores */}
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                {caja && (
                  <div
                    className={`self-start md:self-center px-4 py-1.5 rounded-full text-[10px] md:text-xs font-black tracking-widest transition-colors ${
        ...
        <p className="mt-8 text-center text-[10px] md:text-xs text-(--c-gray) opacity-60 px-4">
          * Una vez que envíes el mensaje, te confirmaremos el stock y el costo
          de envío.
        </p>
      </div>
    </section>
