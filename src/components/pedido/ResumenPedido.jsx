      ...
      <div className="p-8 space-y-6">
        ...
        <div className="flex justify-between items-end border-b border-dashed border-gray-200 pb-6">
          <div className="space-y-1">
            <p className="text-xs text-gray-400 uppercase font-bold tracking-tighter">Capacidad de la caja</p>
            <p className="text-2xl font-black text-(--c-primary)">{cantidad} Alfajores</p>
          </div>
          <div className="text-right space-y-1">
            <p className="text-xs text-gray-400 uppercase font-bold tracking-tighter">Estado</p>
            <p className={`text-lg font-mono font-bold ${faltaRellenar === 0 ? 'text-green-500' : 'text-orange-400'}`}>
        ...
        <div className="space-y-3">
          {items.length > 0 ? (
            <div className="space-y-2">
              {items.map(([sabor, qty]) => (
                <div key={sabor} className="flex justify-between items-center group">
        ...
        {faltaRellenar > 0 && (
          <div className="bg-orange-50 p-4 rounded-xl flex items-center gap-3 border border-orange-100">
            ...
            <p className="text-xs text-orange-700 font-medium">
              Te faltan <span className="font-bold underline">{faltaRellenar}</span> alfajores para completar tu caja de {cantidad}.
            </p>
          </div>
        )}
        ...
        {faltaRellenar < 0 && (
          <div className="bg-red-50 p-4 rounded-xl flex items-center gap-3 border border-red-100">
            ...
            <p className="text-xs text-red-700 font-medium">
              Tenés <span className="font-bold underline">{Math.abs(faltaRellenar)}</span> de más. Sacá algunos para poder pedir.
            </p>
          </div>
        )}
      </div>
